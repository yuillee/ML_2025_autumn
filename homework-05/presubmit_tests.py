...
def test_QuadraticOracle():
    ...
    x = np.zeros(3)
    assert np.allclose (quadratic.func(x), 0.0)
    ...
    x = np.ones(3)
    assert np.allclose (quadratic.func(x), -4.5)
    ...
    x = np.ones(3)
    d = -np.ones(3)
    assert np.allclose (quadratic.func_directional(x, d, alpha=0.5),
                        -2.625)
    assert np.allclose (quadratic.grad_directional(x, d, alpha=0.5),
                        4.5)
    assert np.allclose (quadratic.func_directional(x, d, alpha=1.0),
                        0.0)
    assert np.allclose (quadratic.grad_directional(x, d, alpha=1.0),
                        6.0)
...
def check_log_reg(oracle_type, sparse=False):
    ...
    x = np.zeros(2)
    assert np.allclose (logreg.func(x), 0.693147180)
    ...
    x = np.zeros(2)
    d = np.ones(2)
    assert np.allclose (logreg.func_directional(x, d, alpha=0.5),
                        0.7386407091095)
    assert np.allclose (logreg.grad_directional(x, d, alpha=0.5),
                        0.4267589549159)
    assert np.allclose (logreg.func_directional(x, d, alpha=1.0),
                        1.1116496416598)
    assert np.allclose (logreg.grad_directional(x, d, alpha=1.0),
                        1.0559278283039)
...
def test_line_search():
    ...
    assert np.allclose(ls_tool.line_search(oracle, x, d, ), 1.0)
    ls_tool = optimization.LineSearchTool(method='Constant', c=10.0)
    assert np.allclose(ls_tool.line_search(oracle, x, d), 10.0)

    # Armijo rule
    ls_tool = optimization.LineSearchTool(method='Armijo', alpha_0=100, c1=0.9)
    assert np.allclose(ls_tool.line_search(oracle, x, d), 12.5)

    ls_tool = optimization.LineSearchTool(method='Armijo', alpha_0=100, c1=0.9)
    assert np.allclose(ls_tool.line_search(oracle, x, d, previous_alpha=1.0), 1.0)

    ls_tool = optimization.LineSearchTool(method='Armijo', alpha_0=100, c1=0.95)
    assert np.allclose(ls_tool.line_search(oracle, x, d), 6.25)
    ...
    assert np.allclose(ls_tool.line_search(oracle, x, d), 16.0)
    ls_tool = optimization.LineSearchTool(method='Wolfe', c1=1e-4, c2=0.8)
    assert np.allclose(ls_tool.line_search(oracle, x, d), 32.0)
